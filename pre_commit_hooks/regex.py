...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    for filename in args.filenames:
        with open(filename, 'rb') as f:
            line = f.read()
            files_containing.append(filename) if re.search(re.compile(args.pattern.encode()), line) else None

    if files_containing:
        color = '\033[31m' if is_critical else '\033[33m'
        icon = emoji.emojize(':no_entry:') if is_critical else emoji.emojize(':bulb:', use_aliases=True)
